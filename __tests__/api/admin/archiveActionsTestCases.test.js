...
describe('Archive Actions API Tests', () => {
  ...
  test('Successfully archive a post', async () => {
    const request = new Request('http://localhost:3000/api/admin/archivePost?postId=123');
    const response = await archivePost(request);
    expect(response.status).toBe(200);
  });

  test('Fail archive with missing postId', async () => {
    const request = new Request('http://localhost:3000/api/admin/archivePost');
    const response = await archivePost(request);
  ...
  test('Successfully unarchive a post', async () => {
    const request = new Request('http://localhost:3000/api/admin/unarchivePost?postId=123');
    const response = await unarchivePost(request);
    expect(response.status).toBe(200);
  });

  test('Fail unarchive with missing postId', async () => {
    const request = new Request('http://localhost:3000/api/admin/unarchivePost');
    const response = await unarchivePost(request);
    expect(response.status).toBe(400);
  });
}); 